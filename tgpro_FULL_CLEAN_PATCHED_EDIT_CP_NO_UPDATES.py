...
# USER STRING SESSION (NO FILE SESSION)
...
async def ensure_peer(app: Client, chat_id: int):
    try:
        await app.get_chat(chat_id)
    except Exception:
        await app.get_dialogs(limit=300)
        await app.get_chat(chat_id)
...
# ================= APPLY EDIT (ORIGINAL MESSAGE) =================
@app.on_message(filters.command("tgprofilters_apply_edit") & owner_only)
async def tgprofilters_apply_edit_cmd(c, m: Message):
    ...
    for mid in ids:
        try:
            msg = await fetch_message(chat_id, mid)
            if not msg:
                continue
            ...
            new = apply_filters(old, FILTERS)

            if new != (old or ""):
                ok = await safe_edit_message(app, chat_id, mid, new)
                if ok:
                    applied += 1
        except Exception as e:
            logger.error("apply_edit error: %s", e)

    await m.reply_text(f"✅ Applied edits: {applied}", quote=True)


# ================= APPLY COPY + EDIT =================
@app.on_message(filters.command("tgprofilters_apply_cp") & owner_only)
async def tgprofilters_apply_cp_cmd(c, m: Message):
    ...
    for mid in ids:
        try:
            ...
            copied = await app.copy_message(DEST_CHANNEL, chat_id, mid)
            if copied and new.strip():
                await safe_edit_message(app, DEST_CHANNEL, copied.id, new)
            ...
            logger.error("apply_cp error: %s", e)
