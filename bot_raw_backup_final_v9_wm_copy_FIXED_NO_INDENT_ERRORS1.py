...
from tgpro_copy import register_copy_handler
...
async def apply_video_watermark(input_path: str, app, reply_chat: int) -> str | None:
    import os
    import time
    import asyncio
    ...
    try:
        ...
        # 🔴 THIS LINE MUST BE AT SAME INDENT LEVEL AS cmd / status_msg
        ...
        while True:
            ...
            if line.startswith("out_time_ms"):
                now = time.time()
                if now - last_update >= 600:  # 10 min
                    ms = int(line.split("=")[1])
                    minutes = ms // 1_000_000 // 60

                    dlog(f"⏳ [WATERMARK] processed ~{minutes} min")

                    if status_msg:
                        try:
                            await status_msg.edit_text(
                                f"🎬 Watermarking in progress...\n⏳ Processed: ~{minutes} min"
                            )
                        except Exception:
                            pass
...
class BackupBotFinalV2:
    ...
    def register_handlers(self):
        ...
        # 🔹 Register universal copy handler (channel/group/topic safe)
...
# Not protected -> smart copy (hide sender, try all methods)
                    try:
                        method = await smart_copy_message(
                            self.app,
                            self.dest_channel,
                            msg_obj
                        )
                        ...
                        if method:
                            forwarded += 1
                            dlog("tgproforward: success via", method, "for", mid)
                        else:
                            failures += 1
                            dlog("tgproforward: ALL copy methods failed for", mid)

                        await asyncio.sleep(random.uniform(1.0, 2.0))

                    except Exception as e_copy:
                        failures += 1
                        dlog("tgproforward: smart copy error for", mid, e_copy)
                except Exception as e:
                    failures += 1
            ...
            for mid in ids:
                try:
                    ...
                    if not msg:
                        ...
                        continue
                    # ownership check (supports channel posts)
                    is_mine = False
                    if getattr(msg, "outgoing", False):
                        is_mine = True
                    elif msg.from_user and msg.from_user.id == owner_me.id:
                        is_mine = True
                    elif msg.sender_chat and msg.sender_chat.id == chat_id:
                        is_mine = True
        ...
        async def tgprofilters_apply_cmd(c, m: Message):
            # 🔹 NEW: direct apply with link / range
            if len(m.command) > 1:
                link = m.command[1].strip()
                parsed = parse_link(link)
                ids = extract_ids_from_link(link)
                ...
                try:
                    ...
                    await m.reply_text("❌ Chat not accessible.", quote=True)
                    return
                ...
                for mid in ids:
                    try:
                        ...
                        old = extract_src_caption(msg)
                        if not old:
                            continue

                        new = apply_filters(old, self.filters)
                        if new == old:
                            continue

                        try:
                            await self.app.edit_message_caption(chat_id, mid, new)
                        except Exception:
                            await self.app.edit_message_text(chat_id, mid, new)
            ...
            # 🔹 OLD BEHAVIOR (preview-based apply)
            try:
            ...
            for mid in ids:
                try:
                    ...
                    is_mine = (
                        ...
                        or (msg.from_user and msg.from_user.id == owner_me.id)
                        or (msg.sender_chat and msg.sender_chat.id == chat_id)
                    )
...
async def addfilter_cmd(c, m: Message):
    if len(m.command) < 3:
        await m.reply_text(
            'Usage: /addfilter <bad> <replacement>\nUse "" (two quotes) as replacement to remove the word.',
            quote=True
        )
        return
...
async def backup_cmd(c, m: Message):
    if len(m.command) < 2:
        await m.reply_text(
            "❌ Provide link. Example: /tgprobackup https://t.me/c/<internal>/<topic?>/<start>-<end>",
            quote=True
        )
        return
    link = m.command[1].strip()
...
async def status_cmd(c, m: Message):
    if self.current_backup:
        ...
        await m.reply_text(
            f"📊 Running: {idx}/{total} in {chat.get('title')} (done {done}). "
...
# filename hint
                ...
                if downloaded and os.path.exists(downloaded):
                    ...
                    async def upload_video(video_path: str):
                        thumb = await generate_video_thumbnail(video_path, second=10)
                    ...
                    try:
                        if med.get("is_photo"):
                            ...
                            if self.watermark_enabled:
                                ...
                                wm = await apply_video_watermark(downloaded, self.app, reply_chat)
                                dlog("🎨 [WATERMARK] watermark output:", wm)
                                if wm:
                                    final_video = wm

                            dlog("⬆️ [UPLOAD] sending video", final_video)
                            thumb = await upload_video(final_video)
                            ...
                            try:
                                if final_video != downloaded and os.path.exists(final_video):
                        ...
                        logger.warning("FloodWait %s sec — retrying", wait)
                        ...
                        try:
                            if med.get("is_video"):
                                ...
                                if self.watermark_enabled:
                                    wm = await apply_video_watermark(
                                        downloaded,
                                        self.app,
                                        reply_chat
                                    )
                                ...
                                dlog("⬆️ [UPLOAD] retry video", final_video)
                                thumb = await upload_video(final_video)

                                # 🧹 CLEANUP AFTER RETRY SUCCESS
                                try:
                                    if thumb and os.path.exists(thumb):
                                        os.remove(thumb)
                                except Exception:
                ...
                if status_msg:
                    await safe_edit(
                        status_msg,
                        f"Processing {i}/{total} — done {done}"
                    )
            ...
            if status_msg:
                await safe_edit(
                    status_msg,
                    f"✅ Completed — processed {done}/{total}"
                )
    ...
    async def start(self):
        ...
        logger.info(
            "Logged in as: %s",
            getattr(me, "first_name", getattr(me, "id", str(me)))
        )
...
async def smart_copy_message(app, dest, msg):
    chat_id = msg.chat.id
    mid = msg.id

    # 1️⃣ Normal copy
    try:
        await app.copy_message(dest, chat_id, mid)
        return "copy_message"
    except Exception:
        pass

    # 2️⃣ Copy with caption
    try:
        ...
        return "copy_message_with_caption"
    except Exception:
        pass
    ...
    if msg.document:
        ...
        # 3️⃣ send_document via file_id
        try:
            await app.send_document(dest, fid, caption=msg.caption)
            return "send_document_file_id"
        except Exception:
            pass

        # 4️⃣ cached media (may fail on some Pyrogram builds, that's OK)
        try:
            await app.send_cached_media(dest, fid, caption=msg.caption)
            return "send_cached_media"
        except Exception:
            pass
        ...
        try:
            ...
            await app.send_media_group(
                dest,
                [InputMediaDocument(media=fid, caption=msg.caption)]
            )
    ...
    # 6️⃣ FINAL fallback
    try:
        await app.forward_messages(dest, chat_id, mid, drop_author=True)
        return "forward_drop_author"
    except Exception:
        return None
