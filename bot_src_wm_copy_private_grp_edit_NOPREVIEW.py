...
# ================= PATCH: NO-PREVIEW APPLY (OWNER ONLY) =================

# This handler overrides preview-based apply.
# It re-parses the link and applies filters directly on all IDs.
@app.on_message(filters.command("tgprofilters_apply") & owner_only)
async def tgprofilters_apply_nopreview(c, m: Message):
    parts = m.text.split(maxsplit=1)
    if len(parts) < 2:
        await m.reply_text("Usage: /tgprofilters_apply <link>")
        return

    chat_id, ids = parse_tg_link(parts[1])
    ...
    for mid in ids:
        try:
            msg = await c.get_messages(chat_id, mid)
            if not msg or not msg.from_user or msg.from_user.id != me.id:
                skipped += 1
                continue
            ...
            new_text = apply_filters(text, filters_map)
            if new_text == text:
                skipped += 1
                continue

            if await safe_edit_message(c, chat_id, mid, new_text):
                applied += 1
